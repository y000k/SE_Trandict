...
#         print(e)
#
# # def Delete(cur, wordList):
# #     for word in wordList:
# #         cur.execute('SELECT abbreviation FROM vocab WHERE vocab = "%s";' % word)
# #         print('SELECT abbreviation FROM vocab WHERE vocab = "%s";' % word)
# #         if 'vocabulary' in str(cur.fetchall()):
# #             cur.execute('UPDATE vocab SET abbreviation = "" WHERE vocab = "%s";' % word)
# #             print('=============================' + 'UPDATE vocab SET abbreviation = "" WHERE vocab = "%s";' % word)
# #             con.commit()
#
# def Delete(cur, wordList):
#     for word in wordList:
#         if word == 'vocab':
#             continue
#         cur.execute('SELECT wyy_additional FROM vocab WHERE vocab = "%s";' % word)
#         print('SELECT wyy_additional FROM vocab WHERE vocab = "%s";' % word)
#         wyy_additional = str(str(cur.fetchall())).replace("[('", '').replace("',)]", '')
#         print(wyy_additional)
#         if ('比较' in wyy_additional) or ('最高' in wyy_additional):
#             wyy_additional = wyy_additional.replace('比较级>, ', '比较级<')
#             wyy_additional = wyy_additional.replace('最高级>, ', '最高级<')
#             cur.execute('UPDATE vocab SET wyy_additional = "%s" WHERE vocab = "%s";' % (wyy_additional, word))
#             print('=============================' + 'UPDATE vocab SET wyy_additional = "%s" WHERE vocab = "%s";' % (wyy_additional, word))
#             print('=============================' + wyy_additional)
#             con.commit()
#
# if __name__ == '__main__':
#     con = sqlite3.connect('Database.db')
#     con2 = sqlite3.connect('RE.db')
#     # YouDao(con, SelectTableField(con, "vocab", "vocab")[189500:])
#     cur = con.cursor()
#     cur2 = con2.cursor()
#     cur2.execute('SELECT vocab,abbreviation  FROM vocab WHERE abbreviation != "词汇 (vocabulary) ";')
#     for i in cur2.fetchall():
#         cur.execute('UPDATE vocab SET abbreviation = "%s" WHERE vocab = "%s";' % (i[1], i[0]))
#         con.commit()
#     # Delete(cur, SelectTableField(con, "vocab", "vocab")[305000:])
#     con.close()
#
#
# """
